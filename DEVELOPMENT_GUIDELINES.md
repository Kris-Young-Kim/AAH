...
### AAH 프로젝트 특화 규칙
- Supabase RLS는 사용하지 않는다. 쿼리/액션 로그를 핵심 지점에 남긴다.
- DeviceOrientation 권한은 사용자 클릭 이벤트 안에서 요청(iOS 필수).
- Pretendard를 글로벌 폰트로 사용한다(`globals.css` CDN 로드).
- WebGazer는 lazy load, 탭 비활성 시 pause. 회귀/트래커 설정은 옵션화.
- R3F/three.js는 Billboard Sprite 기반 가벼운 마커를 우선 사용, 드리프트 리셋 버튼을 둔다.
- 핵심 액션(syncUser, saveDevice, toggleDeviceStatus)은 실패/성공 로그를 남긴다.
